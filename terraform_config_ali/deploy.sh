...
create_infrastructure() {
    ...
    # 导出Ansible清单
    terraform output -json ansible_inventory > "../${ANSIBLE_DIR}/${INVENTORY_FILE}"
...
wait_for_ssh() {
    echo "等待SSH服务就绪..."
    HOST=$(jq -r '.all.hosts.tenos.ansible_host' "${ANSIBLE_DIR}/${INVENTORY_FILE}")
    USER=$(jq -r '.all.hosts.tenos.ansible_user' "${ANSIBLE_DIR}/${INVENTORY_FILE}")
    PASSWORD=$(jq -r '.all.vars.ansible_ssh_pass' "${ANSIBLE_DIR}/${INVENTORY_FILE}")
